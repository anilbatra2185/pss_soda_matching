...
from misc.softdp.dp_utils import VarTable, minGamma, minProb
# from dp_utils import VarTable, minGamma, minProb
...
def SoftSODA(cost_matrix, softning='prob',
    ...
    def min_fn(x): return _min_fn(x, gamma=gamma_min)

    # initialize soft-SODA table
    _, nrows, ncols = cost_matrix.shape
    # sdtw = torch.zeros((nrows+1,ncols+1)).to(torch.float).to(device)
    sdtw = VarTable((nrows + 1, ncols + 1))

    path = -np.ones((nrows + 1, ncols + 1))
    ...
    for i in range(1, nrows + 1):
        for j in range(1, ncols + 1):
            ...
            neighbors = torch.stack(
                [sdtw[i, j - 1], sdtw[i - 1, j], sdtw[i - 1, j - 1] + cost_matrix[0, di, dj]])
            new_val = min_fn(neighbors)
            sdtw[i, j] = torch.squeeze(new_val, 0)
            with torch.no_grad():
                path[i, j] = torch.argmin(neighbors)
    
    sdtw_loss = sdtw[nrows, ncols]
    return sdtw_loss, np.transpose(path[1:, 1:])
